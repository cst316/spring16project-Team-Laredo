...
class TaskListVersioning {
    ...
    private static DocType getCurrentDocType() {
        return new DocType("tasklist", TaskListVersioning.getCurrentVersionPublicId(), "tasklist.dtd");
    }

    private static String getCurrentVersionPublicId() {
        return VERSIONS[VERSIONS.length - 1];
    }

    private static int getIndexOfVersion(String publicId) {
        if (publicId == null) {
    ...
    private static void upgrade1_1d1(String[] projectIds) {
        for (String projectId : projectIds) {
            Util.debug("Upgrading project " + projectId + " from version 1.0 to version 1.1d1");
