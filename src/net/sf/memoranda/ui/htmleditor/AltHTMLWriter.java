...
public class AltHTMLWriter extends AbstractWriter {
     ...
     * be emitted.
     */
    private final Stack<Element> blockElementStack = new Stack<>();
    private final boolean completeDoc;
    /*
     ...
     */
    private AltHTMLWriter(Writer w, HTMLDocument doc, int len, String enc, boolean nument) {
        super(w, doc, 0, len);
        completeDoc = (len == doc.getLength());
        setLineLength(80);
        this.encoding = enc;
        this._nument = nument;
    }
     ...
     */
    private static int[] extractCoords(Object stringCoords) {
     ...
     *            location within the document.
     */
    private void emptyTag(Element elem) throws BadLocationException, IOException {
     ...
     */
    private boolean isBlockTag(AttributeSet attr) {
     ...
     * @exception IOException on any I/O error
     */
    private void startTag(Element elem) throws IOException, BadLocationException {
     ...
     *            location within the document.
     */
    private void textAreaContent(AttributeSet attr) throws BadLocationException, IOException {
     ...
     * @exception IOException on any I/O error
     */
    private void selectContent(AttributeSet attr) throws IOException {
     ...
     *
     */
    private void writeOption(Option option) throws IOException {
     ...
     * @exception IOException on any I/O error
     */
    private void endTag(Element elem) throws IOException {
        if (synthesizedElement(elem)) {
            return;
        }
     ...
     *            location within the document.
     */
    private void comment(Element elem) throws BadLocationException, IOException {
     ...
     */
    private void writeAdditionalComments() throws IOException {
     ...
     */
    private boolean synthesizedElement(Element elem) {
     ...
     */
    private boolean matchNameAttribute(AttributeSet attr, HTML.Tag tag) {
        Object o = attr.getAttribute(StyleConstants.NameAttribute);
        if (o instanceof HTML.Tag) {
            HTML.Tag name = (HTML.Tag) o;
            if (name == tag) {
     ...
     * @exception IOException on any I/O error
     */
    private void writeEmbeddedTags(AttributeSet attr) throws IOException {
     ...
     * @exception IOException on any I/O error
     */
    private void closeOutUnwantedEmbeddedTags(AttributeSet attr) throws IOException {
     ...
     */
    private void writeStyles(StyleSheet sheet) throws IOException {
        if (sheet != null) {
     ...
     */
    private boolean writeStyle(String name, Style style, boolean outputStyle) throws IOException {
    ...
    private void writeStyleStartTag() throws IOException {
        indent();
    ...
    private void writeStyleEndTag() throws IOException {
        decrIndent();
        indent();
        write("-->");
        writeLineSeparator();
