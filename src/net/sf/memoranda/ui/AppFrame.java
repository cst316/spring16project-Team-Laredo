...
public class AppFrame extends JFrame {
	...
	JMenuItem jMenuFileNewNote = new JMenuItem(
	...
	private void jbInit() throws Exception {
		this.setIconImage(new ImageIcon(AppFrame.class
				.getResource("resources/icons/jnotes16.png")).getImage());
		...
		jMenuHelpGuide.setIcon(new ImageIcon(AppFrame.class
				.getResource("resources/icons/help.png")));
		...
		jMenuHelpWeb.setIcon(new ImageIcon(AppFrame.class
		...
		jMenuInsertChar.setToolTipText(Local
				.getString("Insert Special character"));
		...
		jMenuFormatProperties.setText(Local.getString("Object properties")
				+ "...");
		jMenuFormatProperties.setToolTipText(Local
				.getString("Object properties"));
		...
		this.workPanel.dailyItemsPanel.alarmB
				.addActionListener(setMenusDisabled);
	...
	public void doPrjPack() {
		java.io.File f = FileDialog.getFile(Local.getString("Pack project"),
				new AllFilesFilter(AllFilesFilter.ZIP),
				"LAST_SELECTED_PACK_FILE", this, true, JFileChooser.FILES_ONLY);
		if (f != null) {
	...
	public void doPrjUnPack() {
		java.io.File f = FileDialog
				.getFile(Local.getString("Unpack project"), new AllFilesFilter(
						AllFilesFilter.ZIP), "LAST_SELECTED_PACK_FILE", this,
	...
	protected void ppExport_actionPerformed(ActionEvent e) {
		...
		NotesExportDialog dlg = new NotesExportDialog(App.getFrame(),
				Local.getString("Export notes") );
		...
		Dimension dlgSize = new Dimension(550, 160);
		dlg.setSize(dlgSize);
		...
		Context.put("EXPORT_TITLES_AS_HEADERS", new Boolean(
		...
		if (ei == 1)
			enc = "UTF-8";
		boolean nument = (ei == 2);
		File f = FileDialog.getFile(Local.getString("Export notes"),
				new AllFilesFilter(AllFilesFilter.HTML),
				"LAST_SELECTED_EXPORT_FILE", this, true,
				JFileChooser.DIRECTORIES_ONLY);
		// boolean xhtml =
		// chooser.getFileFilter().getDescription().indexOf("XHTML") > -1;
		if (f != null) {
			CurrentProject.save();
			ProjectExporter.export(CurrentProject.get(), f, enc, false,
	...
	protected void ppImport_actionPerformed(ActionEvent e) {
		...
		try {
			...
			Element body = document.getRootElement().getFirstChildElement(
					"body");
			...
			for (int i = 0; i < namelist.size(); i++) {
				item = namelist.get(i);
				id = item.getFirstChildElement("a").getAttributeValue("href")
						.replace("\"", "").replace("#", "");
				name = item.getValue();
			...
			for (Map.Entry<String, String> entry : notesName.entrySet()) {
				...
				Note note = CurrentProject.getNoteList().createNoteForDate(
						CurrentDate.get());
				note.setTitle(name);
	...
	protected void p1Import_actionPerformed(ActionEvent e) {
		...
		try {
			Document document = parser.build(f);
			content = document.getRootElement().getFirstChildElement("body")
					.getValue();
